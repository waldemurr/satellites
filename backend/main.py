...
def create_app():
    ...
    def upload_data_file():
        ...
        try:
            ...
            if request.files and 'file' in request.files:
                ...
                if not SAFE_NAME.match(filename):
                    return jsonify({'error': 'Имя файла должно заканчиваться на .json'}), 400
                ...
                if not isinstance(scenario, dict):
                    return jsonify({'error': 'В теле запроса ожидается {"filename": ..., "scenario": {...}}'}), 400
    ...
    def get_data_file(filename):
        try:
            ...
            return jsonify({'error': f'Файл {filename} содержит некорректный JSON'}), 500
        except ValueError as e:
            return jsonify({'error': str(e)}), 400
        except FileNotFoundError as e:
            return jsonify({'error': str(e)}), 404
    ...
    def validate_scenario():
        ...
        scenario = body.get('scenario')
        if not isinstance(scenario, dict):
            return jsonify({'error': 'В теле запроса ожидается {"scenario": {...}}'}), 400
        errors = validation_errors(scenario)
    ...
    def calculate_route(filename, client_id, gateway_id):
        ...
        try:
            ...
            if strategy not in ('hops', 'latency', 'capacity'):
                return jsonify({'error': 'strategy должен быть hops, latency или capacity'}), 400
            ...
            out = {'t_s': t_s, 'strategy': strategy, 'client': client_id, 'gateway': gateway_id,
                   'connected': res is not None}
            if res:
                out.update(res)
            else:
                reason = routing.classify_no_path(scenario, snap, t_s, client_id, gateway_id)
                out['reason'] = reason
    ...
    def route_timeline(filename, client_id, gateway_id):
        ...
        try:
            ...
            return jsonify({
                'client': client_id, 'gateway': gateway_id, 'strategy': strategy,
    ...
    def satellite_criticality(filename):
        ...
        try:
            ...
            hits = routing.satellite_criticality(scenario, clients, gateways,
                                                 step_sample=step_sample, strategy=strategy)
            return jsonify({
                'strategy': strategy,
                'step_sample': step_sample,
                'criticality': [{'satellite': sat, 'failures_caused': n} for sat, n in hits[:15]],
            })
        except ValueError as e:
            return jsonify({'error': str(e)}), 400
        except FileNotFoundError as e:

...
def _build_adjacency(
    snap: dict, client_id: str, gateway_id: str, exclude_sats: frozenset = frozenset()
):
...
def find_path(
        ...
        for u, edges in adj.items():
            for v, dist_km in edges:
                if u in (client_id, gateway_id) or v in (client_id, gateway_id):
                    ...
                    frac = (
                        ground_link_max_range(el, e["min_elevation_deg"])
                        if el is not None
                        else 0.0
                    )
    ...
    return {
        ...
        "total_dist_km": round(total_km, 1),
    }
...
def classify_no_path(
    s: dict, snap: dict, t_s: float, client_id: str, gateway_id: str
) -> str:
    e = s["environment"]
    sat_ids = {sat["id"] for sat in snap["satellites"] if sat["active"]}

    gw_offline = any(
        o["gateway_id"] == gateway_id and o["start_s"] <= t_s < o["end_s"]
    ...
    client_view = {
        sid
        for sid, el in snap["elevation_deg"].get(client_id, {}).items()
    ...
    gw_view = {
        sid
        for sid, el in snap["elevation_deg"].get(gateway_id, {}).items()
        if sid in sat_ids and el >= e["min_elevation_deg"]
    }
    if not gw_view:
...
class RouteTimeline:
    ...
    )  # [{t, connected, path?, reason?, latency_ms?}]
    outages: list = field(
        default_factory=list
...
def compute_route_timeline(
    s: dict, client_id: str, gateway_id: str, strategy: str = "hops"
) -> RouteTimeline:
    e = s["environment"]
    ...
    for t in times:
        ...
        if res is not None:
            ...
            if cur_outage_start is not None:
                tl.outages.append(
                    {
                        "start_s": cur_outage_start,
                        "end_s": t,
                        "duration_s": t - cur_outage_start,
                        "reason": cur_outage_reason,
                    }
                )
                cur_outage_start = None
        else:
        ...
        tl.outages.append(
            {
                "start_s": cur_outage_start,
                "end_s": end,
                "duration_s": end - cur_outage_start,
                "reason": cur_outage_reason,
            }
        )
        ...
        for cid in client_ids:
            for gid in gateway_ids:
                ...
                for sat in sat_nodes:
                    checked += 1
                    alt = find_path(
                        s, snap, cid, gid, strategy, exclude_sats=frozenset({sat})
                    )
                    if alt is None:
                        hits[sat] += 1
...
def find_backup_path(
    s: dict, snap: dict, client_id: str, gateway_id: str, strategy: str = "hops"
):
    primary = find_path(s, snap, client_id, gateway_id, strategy)
    if primary is None:
        return primary, None
    used_sats = frozenset(
        n for n in primary["path"] if n not in (client_id, gateway_id)
    )
...
def scenario_routing_report(
    s: dict, strategy: str = "hops", criticality_sample: int = 6
) -> dict:
    ...
    report["criticality"] = satellite_criticality(
        s, clients, gateways, step_sample=criticality_sample, strategy=strategy
    )[:10]
    return report
